...
struct Tree {
    ...
    TreeNode* findDeepestBlock(TreeNode* currentNode){// finding the longest chain in the tree
        if (!currentNode || currentNode->children.empty()) {// if leaf node
            return currentNode;
        }

        // Find the child with the maximum depth recursively
        auto maxDepthChild = max_element(currentNode->children.begin(), currentNode->children.end(),
            [this](const TreeNode* a, const TreeNode* b) {
                return a->block.depth < b->block.depth;
            });

        // Recursively search in the subtree with the maximum depth child
        return findDeepestBlock(*maxDepthChild);
    }

    vector<Block> longest_chain(){
        // Find the block with the maximum depth
        vector<Block> result;
        if(root->children.size()==0){
            result.push_back(root->block);
            return result;
        }
        // given root find the maximum 
        TreeNode *maxDepthBlock = findDeepestBlock(root);
        // Build the longest chain starting from the block with maximum depth
        result.push_back((maxDepthBlock)->block);
        while (result.back().parent_id != -1) {
            int parentId = result.back().parent_id;
            TreeNode* parentBlock = findNode(root, parentId);
            result.push_back(parentBlock->block);
        }

        reverse(result.begin(), result.end());  // Reverse the vector to get the longest chain in order
        return result;
    }
    ...
    void printAllNodes_file(TreeNode* currentNode, fstream& file){
        ...
        // Print the current node
        file << currentNode->block.block_id <<" "<<currentNode->block.parent_id<< " " << currentNode->block.time << endl;
        // Recursively print all child nodes
        for (TreeNode* child : currentNode->children) {
...
{//storing all the details of the peers
    ...
    peer(){}
    peer(int ID,bool lowcpu,bool fast)//constructor to store all information of the peer
    {
        this->ID=ID;
        this->lowcpu=lowcpu;
        this->slow = !fast;
    ...
    vector<Block> get_longest_chain(){
        return longest_chain;// Current longest chain
    }
    ...
    bool add_block(Block b){//Adding block to the tree and updating the longest chain and updating transaction pool
        ...
        longest_chain = BlockChain.longest_chain();// Post the recursive adding of new childrens of recieved block longest chain 
        if(!same_chain(prev_longest_chain)){// checing if the longest chain is same else updating transaction
            ...
            return false;// same longest chain
        }
    ...
    Block create_block(int block_id,int txn_id,double time){
        ...
        longest_chain = BlockChain.longest_chain();
